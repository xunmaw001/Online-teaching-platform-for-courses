            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">回答管理</li>
                        <li class="breadcrumb-item active">编辑回答</li>
                    </ol>
                </div>
            </div>
        </div>
                            ...
                            <!-- 级联表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 zuoye">
                                        ...
                                        <input style="width: 450px" id="zuoyeUuidNumber" name="zuoyeUuidNumber" class="form-control"
                                               placeholder="作业唯一编号" readonly>
                                    </div>
                                    <div class="form-group col-md-6 zuoye">
                                        <label>作业名称</label>
                                        <input style="width: 450px" id="zuoyeName" name="zuoyeName" class="form-control"
                                               placeholder="作业名称" readonly>
                                    </div>
                                    <div class="form-group col-md-6 zuoye">
                                        <label>作业类型</label>
                                        <input style="width: 450px" id="zuoyeValue" name="zuoyeValue" class="form-control"
                                               placeholder="作业类型" readonly>
                                    </div>
                            <!-- 当前表的字段 -->
                                    <input id="updateId" name="id" type="hidden">
                                <input id="zuoyeId" name="zuoyeId" type="hidden">
                                <input id="xueshengId" name="xueshengId" type="hidden">
                                    <div class="form-group col-md-6 huifuFileDiv">
                                        <label>回答文件</label>
                                        <input name="file" type="file" class="form-control-file" id="huifuFileupload">
                                        <label id="huifuFileName"></label>
                                        <input name="huifuFile" id="huifuFile-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6 huidaDefenDiv">
                                        <label>作业得分</label>
                                        <input style="width: 450px" id="huidaDefen" name="huidaDefen" class="form-control"
                                               onchange="huidaDefenChickValue(this)" placeholder="作业得分">
                                    </div>
                                    <div class="form-group  col-md-6 huidaContentDiv">
                                        <label>批改意见</label>
                                        <input id="huidaContentupload" name="file" type="file">
                                        <script id="huidaContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var huidaContentUe = UE.getEditor('huidaContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="huidaContent" id="huidaContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#huifuFileupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                document.getElementById('huifuFile-input').setAttribute('value', baseUrl + 'file/download?fileName=' + data.result.file);
                document.getElementById('huifuFileName').innerHTML = "上传成功!";
            }
        });


        $('#huidaContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('huidaContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("huida/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addhuida');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.huidaContent != null && ruleForm.huidaContent != 'null' && ruleForm.huidaContent != '' && $("#huidaContentupload").length>0) {

            var huidaContentUeditor = UE.getEditor('huidaContentEditor');
            huidaContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.huidaContent != null){
                    mes = ''+ ruleForm.huidaContent;
                    // mes = mes.replace(/\n/g, "<br>");
                }
                huidaContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#huidaContentupload").length>0) {
            var huidaContentEditor = UE.getEditor('huidaContentEditor');
            if (huidaContentEditor.hasContents()) {
                $('#huidaContent-input').attr('value', huidaContentEditor.getContent());
            }
        }
    }
    //数字检查
        <!-- 当前表的数字检查 -->
        function huidaDefenChickValue(e){
    ...
    // 获取当前详情
    function getDetails() {
        var addhuida = window.sessionStorage.getItem("addhuida");
        if (addhuida != null && addhuida != "" && addhuida != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("huida/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#huidaDefen").val(ruleForm.huidaDefen);
        $("#huidaContent").val(ruleForm.huidaContent);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $(".huidaDefenDiv").remove();//删除当前用户的信息
            $(".huidaContentDiv").remove();//删除当前用户的信息
        }
		else if (window.sessionStorage.getItem('role') == '老师') {
            $(".aaaaaa").remove();
            $(".laoshi").remove();//删除当前用户的信息
            $(".huifuFileDiv").remove();//删除当前用户的信息
        }
        else{
            // alert("未知情况.......");
