        ...
        <div class="ng-inmod ng-fault ng-paddor ng-bg-fff">
            <div class="ng-inmod-ini">
                    <div class="index_titlebox">
                        <h3 class="index_title">教学资源收藏展示</h3>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">教学资源编号</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="jiaoxueziyuanUuidNumber" id="jiaoxueziyuanUuidNumber" placeholder="教学资源编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">教学资源名称</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="jiaoxueziyuanName" id="jiaoxueziyuanName" placeholder="教学资源名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                                                                                                                         
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">教学资源类型</label>
                              <div class="layui-input-inline">
                                  <select name="jiaoxueziyuanTypes" id="jiaoxueziyuanTypes" lay-filter="jiaoxueziyuanTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in jiaoxueziyuanTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div style="margin-top: 2em;">
                        <div v-for="(item,index) in dataList" v-bind:key="index"  class="col-md-3 list-grid">
                            <i @click="deleteJiaoxueziyuanCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                            <div @click="jumpCheck('../jiaoxueziyuan/detail.jsp?id='+item.jiaoxueziyuanId , item.jiaoxueziyuanDelete == null?'':item.jiaoxueziyuanDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="list-img">
                                <img :src="item.jiaoxueziyuanPhoto?item.jiaoxueziyuanPhoto.split(',')[0]:''" class="img-responsive" style="height: 350px;width:100%;" />
                                <div class="textbox"></div>
                                <h4 >名称:{{item.jiaoxueziyuanName}}</h4>
                                <p v-if="item.jiaoxueziyuanNewMoney != null" style="color: red;" >￥{{item.jiaoxueziyuanNewMoney}}</p>
                            </div>
                        </div>
                    </div>
                    <nav class="pages">
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            jiaoxueziyuanCollectionTypesList : [],
        // 级联表字典表数据
            jiaoxueziyuanTypesList : [],
            shangxiaTypesList : [],
            jiaoxueziyuanCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteJiaoxueziyuanCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('jiaoxueziyuanCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function jiaoxueziyuanCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jiaoxueziyuan_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.jiaoxueziyuanCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function jiaoxueziyuanTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jiaoxueziyuan_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.jiaoxueziyuanTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#jiaoxueziyuanUuidNumber').val() != null && jquery('#jiaoxueziyuanUuidNumber').val() != "" && jquery('#jiaoxueziyuanUuidNumber').val() != "null" ) {
                param['jiaoxueziyuanUuidNumber'] = jquery('#jiaoxueziyuanUuidNumber').val();
            }
                     
            if (jquery('#jiaoxueziyuanName').val() != null && jquery('#jiaoxueziyuanName').val() != "" && jquery('#jiaoxueziyuanName').val() != "null" ) {
                param['jiaoxueziyuanName'] = jquery('#jiaoxueziyuanName').val();
            }
                                                                                                                         
            var jiaoxueziyuanTypes = $("#jiaoxueziyuanTypes").find("option:selected").val();
            if (jiaoxueziyuanTypes != null && jiaoxueziyuanTypes != "" && jiaoxueziyuanTypes != "null") {
                            param['jiaoxueziyuanTypes'] = jiaoxueziyuanTypes;
                        }
                                                                                                    
            // 当前表
                                                
            //收藏只能查类型为1的
            param['jiaoxueziyuanCollectionTypes'] = 1;

            // 获取列表数据
            http.request('jiaoxueziyuanCollection/page', 'get', param, function(res) {
